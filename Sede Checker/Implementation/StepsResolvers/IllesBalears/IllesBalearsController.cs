...
{
    class IllesBalearsController : IStepsController
    {
        ...
        private string MallorcaProcedure = "SOLICITUD AUTORIZACIONES - MALLORCA"; //todo: Check it if don't need -> remove it!
        ...
        public IllesBalearsController(SedeTaskData user, 
        ...
        private bool DoEightStep()
        {
            if(this.UserData.ProcedureName.Equals(MallorcaProcedure))
            {
                if (this.WebDriver.ElementExists("#VistaMapa_Datatable"))
                {
                    this.CreateHtmlPage("calendar html page");
                    if (this.WebDriver.ExecuteScript(_scripts["SitaCalendarDateSelector"]) && this.WebDriver.SolveAlertWindow())
                    {
                        return true;
                    }
                    else
